...
import { PurchaseOrderFile } from '@/hooks/usePurchaseOrders';

interface POFilesTabProps {
  poId: string;
}
...
export const POFilesTab = ({ poId }: POFilesTabProps) => {
  const queryClient = useQueryClient();
  const [isUploading, setIsUploading] = useState(false);

  const { data: files, isLoading } = useQuery({
    queryKey: ['po-files', poId],
    queryFn: async () => {
      ...
      return data as PurchaseOrderFile[];
    },
    enabled: !!poId,
  });

  const deleteFile = useMutation({
    mutationFn: async (file: PurchaseOrderFile) => {
      const fileName = file.file_url.split('/').pop();
      if (fileName) {
        await supabase.storage
          .from('purchase-order-files')
          .remove([`${poId}/${fileName}`]);
      }
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['po-files', poId] });
