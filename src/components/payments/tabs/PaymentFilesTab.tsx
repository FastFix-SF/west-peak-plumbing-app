...
interface PaymentFilesTabProps {
  paymentId: string;
}
...
export const PaymentFilesTab = ({ paymentId }: PaymentFilesTabProps) => {
  const queryClient = useQueryClient();
  const [isUploading, setIsUploading] = useState(false);

  const { data: files, isLoading } = useQuery({
    queryKey: ['payment-files', paymentId],
    queryFn: async () => {
  ...
  const deleteFile = useMutation({
    mutationFn: async (file: PaymentFile) => {
      // Delete from storage
      const fileName = file.file_url.split('/').pop();
      if (fileName) {
        await supabase.storage
          .from('payment-files')
          .remove([`${paymentId}/${fileName}`]);
      }
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['payment-files', paymentId] });
